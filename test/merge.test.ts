...
const base = (over: Partial<Stats>): Stats => ({
  ...
  baseline: null, vault: { address: "0xv", sweptDot: 0, unsweptEarned: 0, sweeps: [] }, wallets: [], onchain: null,
...
describe("mergeStats", () => {
  it("keeps wallet balances from the process that last read the chain, even if a fresher process failed", () => {
    ...
    const good = base({ generatedAt: "2026-09-16T23:09:00Z", wallets: [{ address: "0xv", dot: 9000, eth: 0, usdc: 0, role: "vault" }], onchain: { dot: 9000, eth: 0, usdc: 0, dotEquivalent: 9000 }, onchainAt: "2026-09-16T23:09:00Z" });
